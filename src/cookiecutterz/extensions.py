...
def load_inherited_templates(context: dict[str, Any], env: Environment) -> Environment:
...
def install_inherited(project_dir: str, context: dict[str, Any]) -> None:
    ...
    public_keys: dict[str, Any] = get_public_keys(_context)
    this_repo: Path = Path(_context.get("_repo_dir", ""))
    this_repo = this_repo if this_repo.is_absolute() else Path.cwd()
    config_dict: dict[str, Any] = config.get_user_config()  # pyright: ignore[reportUnknownMemberType]
    ancestors_templates: list[str] = []
